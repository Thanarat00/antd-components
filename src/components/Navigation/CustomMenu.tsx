import React from 'react';
import { Menu, MenuProps } from 'antd';
import type { ItemType } from 'antd/es/menu/interface';
import { cn } from '../../utils/cn';

export interface CustomMenuItemType {
  key: string;
  label: string;
  icon?: React.ReactNode;
  disabled?: boolean;
  danger?: boolean;
  children?: CustomMenuItemType[];
  type?: 'group' | 'divider';
  onClick?: () => void;
}

export interface CustomMenuProps extends Omit<MenuProps, 'items'> {
  items: CustomMenuItemType[];
  variant?: 'horizontal' | 'vertical' | 'inline';
  collapsed?: boolean;
  selectedKey?: string;
  openKeys?: string[];
  onSelect?: (key: string) => void;
  onOpenChange?: (keys: string[]) => void;
  className?: string;
}

/**
 * CustomMenu - Enhanced Menu component
 */
export const CustomMenu: React.FC<CustomMenuProps> = ({
  items,
  variant = 'vertical',
  collapsed = false,
  selectedKey,
  openKeys,
  onSelect,
  onOpenChange,
  className,
  ...props
}) => {
  // Transform items
  const menuItems: ItemType[] = React.useMemo(() => {
    const transform = (items: CustomMenuItemType[]): ItemType[] => {
      return items.map((item) => {
        if (item.type === 'divider') {
          return { type: 'divider', key: item.key };
        }
        if (item.type === 'group') {
          return {
            type: 'group',
            key: item.key,
            label: item.label,
            children: item.children ? transform(item.children) : undefined,
          };
        }
        return {
          key: item.key,
          icon: item.icon,
          label: item.label,
          disabled: item.disabled,
          danger: item.danger,
          children: item.children ? transform(item.children) : undefined,
        };
      });
    };
    return transform(items);
  }, [items]);

  const handleClick: MenuProps['onClick'] = ({ key }) => {
    // Find item and call its onClick
    const findItem = (items: CustomMenuItemType[]): CustomMenuItemType | undefined => {
      for (const item of items) {
        if (item.key === key) return item;
        if (item.children) {
          const found = findItem(item.children);
          if (found) return found;
        }
      }
      return undefined;
    };

    const clickedItem = findItem(items);
    clickedItem?.onClick?.();
    onSelect?.(key);
  };

  return (
    <Menu
      mode={variant === 'horizontal' ? 'horizontal' : collapsed ? 'vertical' : 'inline'}
      inlineCollapsed={collapsed}
      items={menuItems}
      selectedKeys={selectedKey ? [selectedKey] : []}
      openKeys={collapsed ? [] : openKeys}
      onClick={handleClick}
      onOpenChange={onOpenChange}
      className={cn(className)}
      {...props}
    />
  );
};

CustomMenu.displayName = 'CustomMenu';

